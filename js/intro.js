export function initIntroCard() {
  document.querySelectorAll('.intro-card').forEach(card => {
    const chk    = card.querySelector('#open-intro');
    ...
    let idx = 0;
    const show = n => { pages.forEach((p,i)=>p.classList.toggle('is-active', i===n)); idx = n; };
    show(0);
    ...
    const next = () => {
      ...
      if (idx === lastIndex) {
        current.classList.add('turn-over');
      ...
      coming.classList.add('pre-active');
      current.classList.add('turn-over');
      current.addEventListener('animationend', function onEnd(){
        current.removeEventListener('animationend', onEnd);
        current.classList.remove('turn-over','is-active');
